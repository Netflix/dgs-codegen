...
class Kotlin2TypeLookup(
     ...
     */
    val operations: Map<String, OperationDefinition.Operation> =
        mapOf(
     ...
     */
    private val builtinScalars: Map<String, ClassName> =
        mapOf(
            Scalars.GraphQLInt.name to INT,
            @Suppress("MaxLineLength", "ktlint:standard:max-line-length")
            Scalars.GraphQLFloat.name
                to DOUBLE, // Float: A signed double-precision floating-point value. https://graphql.org/learn/schema/#scalar-types
     ...
     */
    private val scalarLookup: Map<String, KtTypeName?> =
        document
            ...
            .associate {
                ...
                if (it.hasDirective(JAVA_TYPE_DIRECTIVE_NAME)) {
                    ...
                    val directive =
                        it.getDirectives(JAVA_TYPE_DIRECTIVE_NAME).singleOrNull()
     ...
     */
    private val mappedTypes: Map<String, KtTypeName> =
        config
            .typeMapping
            .mapValues { (_, type) ->
                parseMappedType(
     ...
     */
    private val enumFields: Map<String, List<String>> =
        document
     ...
     */
    private val interfaceFields: Map<String, List<String>> =
        document
     ...
     */
    private val invertedInterfaceLookup: Map<String, List<String>> =
        document
            .getDefinitionsOfType(InterfaceTypeDefinition::class.java)
     ...
     */
    private val invertedUnionLookup: Map<String, List<String>> =
        document
     ...
     */
    fun interfaceImplementors(interfaceName: String): List<String> = invertedInterfaceLookup[interfaceName] ?: emptyList()
     ...
     */
    fun isScalar(typeName: String) = builtinScalars.contains(typeName) || enumFields.contains(typeName) || scalarLookup.contains(typeName)
