...
class GraphQLQueryRequest @JvmOverloads constructor(
    ...
    fun serialize(): String {
        ...
        if (projection != null) {
            val selectionSetFromProjection = if (projection is BaseSubProjectionNodeNext<*, *, *> && projection.root() != null) {
                projectionSerializer.toSelectionSet(projection.root() as BaseProjectionNode)
            } else if (projection is BaseSubProjectionNode<*, *> && projection.root() != null) {
                projectionSerializer.toSelectionSet(projection.root() as BaseProjectionNode)
            } else {
